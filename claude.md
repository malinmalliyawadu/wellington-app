...
| `sharing.ts` | Deep link URL generators (`wellington://`), share helpers, website URL support (`welly.nz`) |
...
- **Deep linking**: URL scheme `wellington://` configured in `app.json`. Expo Router auto-handles incoming deep links. Share helpers generate both `wellington://` deep links and `https://welly.nz` web URLs.
- **Share extension**: `expo-share-intent` registers the app as an iOS/Android share target. Incoming URLs (both `wellington://` and `welly.nz`) are parsed and navigated to in `AuthGate`. External content (images, videos, text) opens the create post screen.
